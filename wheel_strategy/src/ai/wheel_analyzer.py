...
from data_processors.csv_processor import WheelDataProcessor
from prompts.wheel_prompts import get_wheel_strategy_prompt, get_system_prompt
...
class WheelStrategyAnalyzer:
    ...
    def __init__(self, aws_region: str = 'us-east-1', data_folder: str = None):
        ...
        # Set data folder relative to wheel_strategy directory
        if data_folder is None:
            wheel_strategy_dir = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
            data_folder = os.path.join(wheel_strategy_dir, 'data')
        
        self.processor = WheelDataProcessor(data_folder)
    ...
    def analyze_ticker(self, ticker: str, date: str = None, 
        ...
        try:
            ...
            if df is None:
                return {'error': f'No data found for {ticker} on {date or "latest date"}'}
            ...
            result = {
                'ticker': ticker,
                'date': date or self.processor.get_available_dates()[-1],
